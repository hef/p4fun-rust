...
//#define HAVE_OPENSSL_VERSION	/* Duplicate. Already defined in curl_config.h */
#endif
...
static size_t ossl_version(char *buffer, size_t size)
{
  ...
  return msnprintf(buffer, size, "%s/%s",
                   OSSL_PACKAGE, OPENSSL_VERSION_STR);
#else
