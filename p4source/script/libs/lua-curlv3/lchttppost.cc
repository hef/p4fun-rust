...
static lcurl_hpost_stream_t *lcurl_hpost_stream_add(lua_State *L, lcurl_hpost_t *p){
  ...
  lcurl_hpost_stream_t *stream = (lcurl_hpost_stream_t*)malloc(sizeof(lcurl_hpost_stream_t));
...
static int lcurl_hpost_add_files(lua_State *L){
  ...
  forms = (curl_forms*)calloc(opt_count + 1, sizeof(struct curl_forms));
  if(!forms){
    return lcurl_fail_ex(L, p->err_mode, LCURL_ERROR_FORM, CURL_FORMADD_MEMORY);
  }
...
static int lcurl_hpost_get(lua_State *L){
  ...
  if(lua_isnoneornil(L, 2)){
    ...
    code = (CURLcode)curl_formget(p->post, &b, lcurl_hpost_getter_by_buffer);
    if(code != CURLE_OK){
  ...
  if(lua_isfunction(L, 2)){
    if(lua_gettop(L) == 2){
      top  = 2;
      code = (CURLcode)curl_formget(p->post, L, lcurl_hpost_getter_by_callback1);
    }
    else{
      top  = 3;
      lua_settop(L, 3);
      code = (CURLcode)curl_formget(p->post, L, lcurl_hpost_getter_by_callback2);
    }
  }
