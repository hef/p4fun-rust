        ...
        double Kp, double Ki, double Kd, int POn, int ControllerDirection)
{
    ...
    // Note:  Perforce modification from original.
    pOnE = pOn == PID_P_ON_E;
    // Note:  Perforce modification from original ^^^^^
    ...
    now.Now();
    lastTime = now.Seconds() * 1000 + now.Nanos() / 1000000;
    // Note:  Perforce modification from original ^^^^^
}
...
bool PID::Compute()
{
   ...
   // Note:  Perforce modification from original.
   DateTimeHighPrecision nowb;
   nowb.Now();
   unsigned long now = nowb.Seconds() * 1000 + nowb.Nanos() / 1000000;
   // Note:  Perforce modification from original ^^^^^
