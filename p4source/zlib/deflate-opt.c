/* Note that this is a modified version of deflate.c */
...
#include <immintrin.h>
/* UBSAN: runtime error: load of misaligned address 0x63100021c801
    ...
    int flush;
{
     ...
     */
           ...
           /* Perforce-local edit for job115766 - avoid a left shift
              in RANK on negative number */
           if (strm->avail_in == 0 && old_flush != -1 &&
...
/* UBSAN: load of misaligned address 0x63100021c80e for
   type 'uint32_t' (aka 'unsigned int'), which requires 4 byte alignment
*/
NO_SANITIZE_UNDEFINED
